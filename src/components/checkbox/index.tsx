...
const CheckboxIcon = ({ status }: { status: boolean }) => {
    ...
    if(status){
        ...
        return (
            <svg width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path fillRule="evenodd" clipRule="evenodd" d="M16 4H8C5.79086 4 4 5.79086 4 8V16C4 18.2091 5.79086 20 8 20H16C18.2091 20 20 18.2091 20 16V8C20 5.79086 18.2091 4 16 4ZM8 2C4.68629 2 2 4.68629 2 8V16C2 19.3137 4.68629 22 8 22H16C19.3137 22 22 19.3137 22 16V8C22 4.68629 19.3137 2 16 2H8Z"/>
            </svg>
        );
...
interface CheckboxProps<T=unknown> {
    ...
    children: string | React.ReactElement | ((checked?: boolean) => React.ReactElement)
...
export const Checkbox = ({ children, checked, disabled, value, onChange }: CheckboxProps) => {

    const groupContext = useCheckboxGroupContext();

    const { values, onChange: groupOnChange, disabled: groupDisabled } = groupContext;

    const status = useMemo(() => !!checked || values.includes(value), [checked, values]);
...
export const CheckboxGroup = <T=unknown>({ values, onChange, children, disabled }: CheckboxGroupProps<T>) => {
